...
export default class InfiniteGrid {
  ...
  onResize() {
    ...
    // Add zoom factor to make everything appear larger (zoomed in)
    const zoomFactor = 2.5; // 2.5x zoom (increased for more zoom in)

    this.tileSize = {
      w: this.winW * zoomFactor,
      h: (this.winW * zoomFactor) * (this.originalSize.h / this.originalSize.w),
    };
    ...
    baseItems.forEach(base => {
      repsX.forEach(offsetX => {
        repsY.forEach(offsetY => {
          ...
          const img = new Image();
          img.src = `/lookbook/${base.src}`;

          // Set dimensions after image loads to maintain aspect ratio
          img.onload = () => {
            const aspectRatio = img.naturalHeight / img.naturalWidth;
            const displayWidth = base.w;
            const displayHeight = displayWidth * aspectRatio;

            itemImage.style.width = `${displayWidth}px`;
            itemImage.style.height = `${displayHeight}px`;

            // Update item dimensions in this.items
            const item = this.items.find(it => it.img === img);
            if (item) {
              item.h = displayHeight;
    ...
    // Calculate center position based on tileSize
    const centerX = -(this.tileSize.w / 2 - this.winW / 2);
    const centerY = -(this.tileSize.h / 2 - this.winH / 2);

    this.scroll.current.x = this.scroll.target.x = this.scroll.last.x = centerX;
    this.scroll.current.y = this.scroll.target.y = this.scroll.last.y = centerY;
  }
