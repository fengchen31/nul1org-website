...
}`;function we(t,e,n=i=>i){let s=new URLSearchParams(window.location.search).get(t);try{if(s!==null)return n(s)}catch{}return e}function $t(t){t=t.replace(/^#/,""),t.length===3&&(t=t.split("").map(r=>r+r).join(""));const e=parseInt(t,16),n=(e>>16&255)/255,i=(e>>8&255)/255,s=(e&255)/255;return[n,i,s]}const Or=we("feed",.018,Number),_f=we("kill",.049,Number);console.log(Or);const xf=Ie(Ue,mf,{pressureMap:{value:0},maskTexture:{value:0},uSize:{value:[0,0]},feed0:{value:Or},kill0:{value:_f}});function yf(t,e,n){xf(t,{pressureMap:e,maskTexture:n,uSize:[t.width,t.height]})}const wt=document.createElement("canvas"),bf=wt.getContext("2d");let bt;function Ef(t,e){return wt.width=t.width,wt.height=t.height,e(wt,bf),bt||(bt=new Gn(t.gl),bt.image=wt),bt.needsUpdate=!0,bt}var wf=`precision highp float;
